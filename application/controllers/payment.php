   ...
   $cari=$this->model_app->getdatapaging("HouseNo,CWT,PCS,Amount,RemainAmount","outgoing_house", "WHERE Shipper='$customer' AND HouseNo='$housecek' AND PaymentStatus='0' ORDER BY HouseNo asc");	 
	...
	foreach($cari as $row){
 	 ...
 	 if ($jumlah >0) { 
		...
		$savedetail=$this->model_app->insert('jurnal',$insert_jurnal);
		...
		$savedetail=$this->model_app->insert('jurnal',$inser_jurnal_header);
...
public function list_journal()
	{
		...
		$select='a.JurnalId,c.nmac,c.kdac,a.PaymentDate,a.JurnalNo,a.Debit,a.Credit,b.Currency,b.Remarks';
