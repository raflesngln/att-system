...
function save_cust_type(){  
	 	$name=$this->input->post('addresstype');
		$desc=$this->input->post('addresstypedesc');
		...
		$tot_hal = $this->model_app->hitung_isi_tabel('*','ms_staff a',"where empInitial like '%$cari%' OR empName like '%$cari%' order by empCode ASC");
			...
			$config['base_url'] = base_url() . 'customer/view_customers/';
		...
		$data['view']='pages/customer/address_type_tabel';
